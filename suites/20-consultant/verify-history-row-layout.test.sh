...
# (hours play no part -- see verify-history-status-badge.test.sh, settled 2026-09-14)
...
if [ "$DELETABLE" -eq 0 ]; then
  ...
  echo "        Expected when every listed week has been submitted; the bug this file guards"
  echo "        appears ONLY on a row that HAS the icon -- i.e. a Draft week, whatever hours"
  echo "        it holds -- so a run without one is weaker coverage than it looks."
  echo "        lib/_seed.sh's fresh-week pool is what supplies one."
fi
