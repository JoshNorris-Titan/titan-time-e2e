...
# 20 sits past every other spec's forward walk and inside the fixture assignment window
# (FX_START_DATE..FX_END_DATE in lib/_fixtures.sh, 07/01/2026 - 12/31/2027).
#
# THAT NUMBER WAS 14 WHEN THIS SPEC WAS WRITTEN (2026-09-10), justified as "past every
# other spec's forward walk -- the longest are 12". That stopped being true: #101
# ("Look far enough ahead for an open week") raised tt_goto_fresh_week's default max
# from 12 to 16, so week+14 is now INSIDE the range the shared fresh-week pool hunts
# through. Five specs draw from that pool and exhausting it is a failure mode this
# suite has already hit, so a spec that parks a saved draft in the middle of the hunt
# is a collision waiting for a slow week. 20 clears 16 with room; if that default moves
# again, this has to move with it.
#
# If this fails after saving and before deleting, it leaves
...
# tt-timeout: 8m
#
# 8m rather than the 4m default. The forward walk is one eval with an in-page loop, so
# it costs one node process rather than twenty -- but that loop allows each week up to
# 10s to re-render, so twenty weeks is up to 200s inside a single call before the two
# reloads, two full gallery loads and the delete round-trip even start. 4m would be a
# cap this spec could hit while working correctly, which is the one thing a timeout
# must never do.
...
AHEAD="${TT_DELETE_WEEKS_AHEAD:-20}"
HOURS="3"
